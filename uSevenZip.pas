...
{$IFDEF WIN64}
  SEVENZIP_DLL_FILENAME = '7z64.dll';
  SEVENZIP_DLL_RESNAME = 'SEVENZIP_DLL_X64';
{$ELSE}
  SEVENZIP_DLL_FILENAME = '7z32.dll';
  SEVENZIP_DLL_RESNAME = 'SEVENZIP_DLL';
{$ENDIF}
...
function AssignSevenZipDll(DllFileName: String): Boolean;
...
begin
  try
    try
      if not FileExists(DllFileName) then begin
        ForceDirectories(ExtractFilePath(DllFileName));
        icsExtractFileFromRes(SEVENZIP_DLL_RESNAME, DllFileName);
      end;
      if FileExists(DllFileName) then _SevenZipDLL := DllFileName else _SevenZipDLL := '';
    except
      _SevenZipDLL := '';
    end;
  finally
    Result := FileExists(_SevenZipDLL);
  end;
end;
...
begin
  ...
  if not (SystemTimeToFileTime(st, Result) and LocalFileTimeToFileTime(Result, Result)) then ; //RaiseLastOSError;
end;
...
var
  st: TSystemTime;
begin
  if not (FileTimeToLocalFileTime(ft, ft) and FileTimeToSystemTime(ft, st)) then ; //RaiseLastOSError;
...
begin
  if hr <> S_OK then raise Exception.Create(SysErrorMessage(hr));
end;
...
function CreateInArchive(const classid: TGUID): I7zInArchive;
begin
  Result := T7zInArchive.Create(_SevenZipDLL);
  Result.ClassId := classid;
end;

function CreateOutArchive(const classid: TGUID): I7zOutArchive;
begin
  Result := T7zOutArchive.Create(_SevenZipDLL);
  Result.ClassId := classid;
end;
